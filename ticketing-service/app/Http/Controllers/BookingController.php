...
class BookingController extends Controller
{
    public function store(Request $request)
    {
        ...
        try {
            // Cari bagian nomor 5 (MINTA VIRTUAL ACCOUNT)
            $responsePayment = Http::timeout(5)->post($urlServiceC, [
                'booking_id'   => $bookingId,
                'amount'       => $totalPrice  // Ganti dari 'total_amount' jadi 'amount'
            ]);

            if ($responsePayment->successful()) {
                // Karena Service C mengirim { "data": { "va_number": "..." } }
                $vaNumber = $responsePayment->json('data.va_number'); 
